...
class AuthenticationController extends GetxController {
  ...
  final RoundedLoadingButtonController introBtnController = RoundedLoadingButtonController();
  ...
  signInResult(LoginRequest data, context)async{
    ...
    if(result?.status == true){
        ...
        SharedPreferences prefs = await SharedPreferences.getInstance();
         ...
         prefs.setString('MobileNumber', result?.details?.contactno ?? '');
         prefs.setString('location', result?.details?.address ?? '');
         prefs.setString('suburb', result?.details?.suburb ?? '') ;
         prefs.setString('city', result?.details?.city ?? '');
         prefs.setString('state', result?.details?.state ?? '');
         prefs.setString('pincode', result?.details?.postcode ?? '');
         resetSelection();
    }
    if(authMode.value != 1) {
        ...
        var snackBar = const SnackBar(
          backgroundColor: Colors.black,
            content: ReusableTextWidget(
              text: 'Please enter your 6 digit verification code provided by PlotRol',
              color: Colors.white,
            ),
        );
        ScaffoldMessenger.of(context).showSnackBar(snackBar);
      }
      Get.to(()=> OtpScreen(otp: resendOtp.value, authMode: authMode.value, logInStatus: logInStatus.value,));
    }
   }
  ...
  onNavigateToProfileNumberVerification({String? otp,authmode=0}){
    otpController.clear();
    resendOtp.value = otp ?? '';
    Get.to(()=> OtpScreen(otp: resendOtp.value, authMode: authmode, logInStatus: logInStatus.value,));
  }
